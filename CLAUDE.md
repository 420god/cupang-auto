...
- **판매수수료: 카테고리별 실제 요율 반영함(2026-08-13, 마이그레이션 006).** WING 수수료안내 페이지(2019-11-25 기준이라 지금과 다를 수 있음)를 파싱해 `categories.commission_rate`를 채움. `web/app.js`의 `commissionFor(catCode)`가 이 값을 우선 쓰고, 매칭 안 된 카테고리만 기존처럼 고정 10.8%로 폴백. **마이그레이션 006은 아직 실제 DB에 실행 안 됨** — 실행 전까지는 여전히 전부 10.8%로 계산됨(폴백 덕분에 깨지진 않음). `docs/decisions.md` 참조.
