...
- 웹사이트 **배포·실사용 확인됨** (2026-08-12). GitHub `420god/cupang-auto` 저장소가 Vercel/Netlify와 연동돼 `main` push 시 자동배포. 최근 배포: `5ad10c5` — 소싱 옵션 상세 표에 입출고비·수수료 컬럼 표시 추가(기존엔 정산/마진 계산에만 내부적으로 쓰였음). 요금표(`state.feeCache`)가 캐시상 12개 구간뿐이라 대부분 상품이 "요금표 없음"으로 표시됨 — 요금표 데이터 보강 필요.
