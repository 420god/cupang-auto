...
내부 역공학 API와 완전히 별개 시스템. 데이터 소스가 네 겹이다:
...
3. **`rocket_growth_profit_daily`** — 역시 WING 내부 API, 수수료·입출고비·보관비·쿠폰·광고비·밀크런까지 전부 **실제 확정값**(추정 아님, 날짜 단위). 단 옵션별이 아니라 **계정 전체 합계만** 나옴.
4. **`rocket_growth_item_cost_snapshots`** — 역시 WING 내부 API(재고현황), 상품별 **개당** 실제 수수료·입출고비·쿠폰비와 이번 달 누적보관비. "상품 원가정보 갱신" 버튼(수동)으로 채움. 덮어쓰지 않고 이력으로 쌓아서 가격 변경 전/후 요율을 구분(`(vendor_item_id, captured_at)` 단위).

왜 이 구조인지(Vercel+프록시를 먼저 시도했다가 버린 이유 포함)는 `docs/decisions.md` 2026-08-13/2026-08-15~16 항목, API 스펙은 `docs/api-notes.md` 4번 섹션. 1·2번은 **날짜 단위로** 병합(그 날짜에 wing이 있으면 wing만, 없으면 1번으로 폴백 — 항목별 병합은 반품으로 순매출 0이 된 항목이 이중 계상되는 버그가 있었음, `docs/troubleshooting.md` 26번), 3·4번은 있으면 추가로 보여줌(대체 관계 아님). 확장프로그램이 없거나 응답 없으면 1번만으로 조용히 폴백 — 이 폴백은 항상 정상 동작해야 하는 핵심 전제, 깨는 수정 하지 말 것.

**UI는 3단 구조다**(상단 고정기간 요약/중단 일별 상세/하단 상품별 상세) — 왜 이렇게 나눴는지, 순이익/영업이익을 어떻게 구분하는지는 `web/CLAUDE.md` 참조. **2026-08-15/16 실사용 검증 완료**(반품·확정정산·상품별 실제원가 전부 화면에서 값이 뜨는 것까지 사용자 직접 확인).

**크롬 확장프로그램은 `chrome://extensions`에서 새로고침해야 반영된다**(git push와 무관) — `extension/background.js`/`interceptor.js`를 고친 세션 다음엔 항상 확인할 것.
