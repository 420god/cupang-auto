...
3. **`rocket_growth_profit_daily`** — 역시 WING 내부 API, 수수료·입출고비·쿠폰·광고비·밀크런까지 전부 **실제 확정값**(추정 아님). 단 옵션별이 아니라 **계정 전체 합계만** 나와서 옵션별 표엔 못 합치고 표 위의 별도 "확정 정산 요약" 카드로만 보여줌.

왜 이 구조인지(Vercel+프록시를 먼저 시도했다가 버린 이유 포함)는 `docs/decisions.md` 2026-08-13 항목, API 스펙은 `docs/api-notes.md` 4번 섹션(4-4-2 반품, 4-4-4 확정 정산). **1번과 2번이 같이 있으면 2번이 우선**(반품 반영), **3번은 있으면 그냥 추가로 보여줌**(대체 관계 아님). 확장프로그램이 없거나 응답 없으면 1번만으로 조용히 폴백 — 이 폴백은 항상 정상 동작해야 하는 핵심 전제, 깨는 수정 하지 말 것.

**2026-08-15 실사용 검증 완료.** 반품·확정정산 둘 다 판매현황 탭에서 실제로 값이 뜨는 것까지 사용자가 직접 확인함. 자세한 흐름은 `docs/api-notes.md` 4-4-2/4-4-4, `extension/CLAUDE.md`("판매현황/반품 동기화" 섹션 — 특히 `x-xsrf-token` 헤더 함정), `web/CLAUDE.md`.
