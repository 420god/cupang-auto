...
- 웹사이트 **배포·실사용 확인됨** (2026-08-12). GitHub `420god/cupang-auto` 저장소가 Vercel/Netlify와 연동돼 `main` push 시 자동배포. 최근 배포: `0c37d9d` — 희망가/원가 자동저장 버그 2건 수정(DB 컬럼 한도 초과 시 400 반복 실패 → 클라이언트에서 차단, 여러 옵션 연달아 편집 시 앞 항목 저장이 조용히 취소되던 공유 디바운스 버그 → `debounceKeyed()`로 item_id별 분리).
