...
웹의 판매현황 탭 (loadSales())
  ...
  → background.js의 onMessageExternal 리스너가 깨어남
     ...
     3. syncProfitForDates: 같은 방식으로 하루씩 profit-status/search 호출
        ...
        — **"빈" 응답(아래 참조)도 실패와 동일하게 저장을 건너뜀**
     4. 로그인 안 돼 있으면(JSON 아닌 응답) notLoggedIn 에러로 즉시 전체 실패 반환
  → 웹에 {ok, days, rowCount, profitRowCount} 응답
```

**함정 1 — 세션 쿠키만으론 부족하고 `x-xsrf-token` 헤더가 따로 필요한 API가 있다(`profit-status/search`, 2026-08-15에 겪고 해결).** 처음엔 그냥 "Failed to fetch"만 보여서 페이지별 CSP 문제로 오인했다 — `https://wing.coupang.com/tenants/rfm/settlements/home` 페이지로 먼저 이동시키는 우회, 백그라운드에서 `credentials:'include'`로 직접 fetch하는 우회 둘 다 시도했지만 소용없었다(그 둘의 흔적이 커밋 히스토리에 남아있을 수 있는데, 최종적으로는 둘 다 필요 없었다). **진짜 원인은 콘솔에 별도로 뜨는 CORS 에러 메시지**(`Access to fetch at 'https://helpseller.coupang.com/access/logout' (redirected from ...) ... blocked by CORS policy`)에만 있었고, 우리 코드가 잡는 `catch(e)`에는 그냥 "Failed to fetch"로만 보였다 — **콘솔 원문을 반드시 같이 볼 것, 우리 쪽 에러 메시지만 보고 판단하지 말 것.** 해결은 `pageFetchProfitStatus()`처럼 쿠키 `XSRF-TOKEN` 값을 `document.cookie`로 읽어서 `x-xsrf-token` 헤더로 그대로 실어 보내는 것 — 쿠키는 페이지 컨텍스트에서만 읽히므로 `chrome.scripting.executeScript`로 페이지 안에서 호출해야 한다(백그라운드 직접 fetch로는 이 쿠키에 못 닿는다). **페이지 이동은 불필요했다** — 인증 헤더만 맞으면 WING 탭이 어느 페이지에 있든 호출된다. **다른 WING 내부 API에서 "Failed to fetch"를 만나면 페이지 이동부터 시도하지 말고, 이 패턴(헤더 캡처 → XSRF 토큰 확인)부터 확인할 것.**

**함정 2 — WING이 그 날짜를 아직 인식 안 했어도 HTTP 200 + `profitAmount:0`인 "빈" 응답을 준다(2026-08-16 발견·수정, `docs/troubleshooting.md` #27).** 주로 당일 자정 직후 자동 동기화("오늘+어제")에서 겪는다 — 정산 인식이 D-1 지연이라 당일치는 아직 아무것도 인식이 안 됐는데도 서버가 에러 대신 필드는 다 있고 값만 전부 0인 응답을 준다. 예전엔 `typeof json.profitAmount === 'undefined'`만 체크해서 이런 빈 응답을 성공으로 오인하고 그대로 `rocket_growth_profit_daily`에 저장했는데, 그 결과 웹의 `buildDailyRow()`가 이 0값을 "확정 정산"으로 믿어버려서 실제 판매가 있는 날짜인데도 매출·수수료·순이익 등이 화면에서 전부 0/공백으로 보이는 버그가 났다(실사용 중 발견 — "판매수량만 맞고 나머지는 다 안 나온다"). 해결: `syncProfitForDates()`에서 `totalSalesAmount`와 `totalDeductionAmount`가 **둘 다 0**이면 빈 응답으로 보고 `failed` 목록에 넣고 저장을 건너뛴다(진짜 무판매일도 있을 수 있지만, 그런 날은 애초에 판매 자체가 없어 웹의 옵션별 추정 폴백도 똑같이 0을 보여주므로 안전). 웹 쪽(`web/app.js`의 `buildDailyRow()`)에도 같은 조건으로 방어 코드를 이중으로 넣어뒀다 — 과거에 이미 저장된 빈 행이나 확장프로그램 없이 다른 경로로 빈 행이 들어올 경우까지 대비.
