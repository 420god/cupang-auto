...
### 입고 화면에 아무것도 안 나온다 (2026-08-18)

**증상**: 발주를 결제완료로 넘겨 "중국배대지 배송중"이 됐는데 **입고의 도착 대기에 아무것도
안 뜬다.** 예전에 "중국배대지 도착"으로 넘긴 건도 중국창고 재고에 없다.

**원인**: **입고 화면은 재고 로트(`inventory_lots`)만 보여준다.** 그런데 청구서 줄에 SKU가
연결되지 않으면 로트가 아예 안 만들어진다(`NOBARCODE`로 발주한 과거 건이 전부 그렇다).
로트가 없으니 입고·출고·원가 어디에도 안 잡히는데, 화면에는 그냥 "없음"으로만 보여서
**원인을 알 방법이 없었다.**

중국창고가 비어 보이는 경우는 하나 더 있다 — 그 로트를 이미 한국으로 출고했으면
`qty_china`가 0이라 정상적으로 안 보인다.

**해결**: 입고 화면 맨 위에 **SKU 미연결 줄 경고**를 넣었다 — 몇 개인지, 어떤 상품인지,
어디서 고치면 되는지(발주 탭에서 SKU 연결)를 같이 알려준다. 빈 목록 문구에도 가능한
이유를 적었다.

**교훈**: 파생 데이터만 보여주는 화면은 **원본이 없을 때 그 사실을 말해줘야 한다.**
