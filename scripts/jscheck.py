...
# 잡는 것 세 가지:
#   1) 괄호 균형
#   2) 따옴표 문자열 안의 진짜 줄바꿈 — 작은/큰따옴표는 줄을 넘을 수 없다.
#      편집 중 개행 이스케이프가 실제 개행으로 바뀌면 여기서 걸린다.
#   3) 블록 주석의 조기 종료 — 주석에 마크다운 강조를 쓰다가 별표 뒤에 /가 오면
#      그 자리에서 주석이 닫히고 뒤가 코드로 해석된다. 2026-08-21에 워커 전체가
#      이걸로 죽었는데, 괄호·따옴표는 멀쩡해서 기존 검사로는 못 잡았다.
import sys, io
...
def check(path):
    ...
    while i < n:
        ...
        if c == '/' and i + 1 < n and src[i+1] == '/':
            ...
            continue
        if c == '/' and i + 1 < n and src[i+1] == '*':
            # 블록 주석. **주석이 의도보다 일찍 끝나는 사고**를 여기서 잡는다.
            # 주석 안에 마크다운 강조(**...**)를 쓰다가 ** 뒤에 /가 오면
            # (예: **/image/) 그 */ 가 주석을 닫아버리고 뒤가 코드로 해석된다.
            # 2026-08-21에 실제로 워커 전체가 이걸로 죽었고, 괄호·따옴표는 멀쩡해서
            # 기존 검사로는 통과했다.
            start_line = line
            i += 2
            while i + 1 < n and not (src[i] == '*' and src[i+1] == '/'):
                if src[i] == '\n': line += 1
                i += 1
            # 닫는 */ 앞이 * 이고(즉 **/), 같은 줄에 그 뒤로 글자가 더 있으면
            # 사람이 의도한 종료가 아니라 사고다 — 진짜 종료는 뒤에 코드가 오거나 줄이 끝난다.
            if i > 0 and src[i-1] == '*':
                rest = src[i+2:src.find('\n', i) if src.find('\n', i) != -1 else n]
                if rest.strip():
                    errors.append(f'{line}행: 주석 안의 "**/" 가 주석을 조기 종료시킨다 '
                                  f'(뒤에 "{rest.strip()[:30]}" 가 코드로 해석됨). '
                                  f'{start_line}행에서 시작한 주석')
            i += 2; continue
