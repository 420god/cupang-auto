...
async function loadLotCogs(vendorItemIds) {
  ...
  /* 여기까지 오면 queues에는 **다 팔고 남은 잔량**이 로트별로 들어 있다(도착 순서 유지).
     상품원장의 마진 미리보기가 이걸 쓴다 — "지금 파는 물건의 원가"는 가장 최근 입고분이
     아니라 **다음에 실제로 나갈 로트**의 원가이기 때문이다(2026-08-20 사용자 확인:
     20개 남은 로트가 다 팔릴 때까지는 그 원가, 그 뒤부터 다음 로트 원가).

     Map에 속성으로 붙이는 이유: 반환 형태를 바꾸면 판매현황 호출부를 같이 고쳐야 하는데,
     그쪽은 이미 검증된 코드다. 무엇보다 **선입선출 계산을 두 벌로 만들지 않으려는 것**이
     핵심이다 — 갈라지면 두 화면의 원가가 조용히 어긋난다.

     ※ 한계: 반품 처리가 아직 거칠다. 음수 수량을 맨 앞 로트에 그대로 되돌리는데,
       실제로는 반품된 물건이 다시 팔리는 등 경우의 수가 더 많다. 사용자가 반품 데이터를
       가져올 경로를 찾아둔 상태라 나중에 정교화 예정 — 그때 이 잔량 계산도 같이 바뀐다. */
  out.remainingBySku = queues;
  return out;
}
