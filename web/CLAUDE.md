...
**상품/옵션별 상세표의 "추정 수수료/입출고비"를 WING 실제값으로 교체하는 작업은 아직 착수 전(2026-08-15 조사 시작)** — 지금은 카테고리 요율표 기반 추정(`commissionFor()`/`feeFor()`)만 쓰는데, 사용자가 WING 재고현황 페이지에서 상품별 "예상(개당)" 툴팁(판매수수료+입출고·배송비용 분해)과 "이번달 누적보관비"를 직접 제공받는 걸 확인함 — 이게 카테고리 추정보다 정확하니 이걸로 대체할 계획. 후보 API 3개(`storage-fee-modal/{vendorItemId}`, `inventory-health-dashboard/search`, `pricing-info/{vendorItemId}`)를 `extension/interceptor.js`의 `SALES_PATHS`에 등록만 해두고 아직 요청/응답 구조는 캡처 전이라 미확정 — 다음 세션에서 이 항목이 남아있으면 사용자에게 캡처 결과부터 물어볼 것(`extension/CLAUDE.md` 참조).
